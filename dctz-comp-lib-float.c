/**
 * @file dctz-comp-lib-float.c
 * @author Seung Woo Son
 * @date July 2019
 * @brief DCTZ compression library routine for float
...
int dctz_compress_float (float *a, int N, size_t *outSize, char *a_z, double error_bound)
{
  ...
  if (NULL == (a_x = (float *)malloc (N*typesize))) {
  ...
  if (NULL == (qtable = (float *)malloc (BLK_SZ*typesize))) {
